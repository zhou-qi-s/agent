...
@router.get("/download_chart")
async def download_chart(
    chart_name: str = Query(..., description="Chart name"),
    chart_version: str = Query(..., description="Chart version"),
):
    """Download Chart file from Harbor via OCI registry API"""
    try:
        conn = get_harbor_connection_info(require_auth=True, project_key="template")
        username = conn["username"]
        password = conn["password"]
        ...
        base_url = f"https://{repository}:443"
        auth = (username, password)

        real_version = chart_version
        # If the exact version is missing, try to resolve a real one from Harbor
        # (DB may record a version that differs from what was actually pushed).
        def _chart_versions():
            try:
                lst = f"{base_url}/api/v2.0/projects/{project}/repositories/{chart_name}/artifacts?page_size=100"
                r = requests.get(lst, auth=auth, verify=False, timeout=30,
                ...
                for a in r.json():
                    for t in (a.get("tags") or []):
                        versions.append(t.get("name"))
                return versions
            except Exception:
                return []

        # Step 1: get OCI manifest
        manifest_url = f"{base_url}/v2/{project}/{chart_name}/manifests/{real_version}"
        headers = {"Accept": "application/vnd.oci.image.manifest.v1+json"}
        resp = requests.get(manifest_url, auth=auth, headers=headers, verify=False, timeout=30)
        if resp.status_code != 200:
            versions = _chart_versions()
            if not versions:
                raise HTTPException(status_code=404, detail=f"Chart {chart_name}:{chart_version} not found")
            # pick best match
            real_version = chart_version if chart_version in versions else versions[0]
            if chart_version:
                for v in versions:
                    if chart_version in v or v in chart_version:
                        real_version = v
                        break
            manifest_url = f"{base_url}/v2/{project}/{chart_name}/manifests/{real_version}"
            resp = requests.get(manifest_url, auth=auth, headers=headers, verify=False, timeout=30)
        ...
        # Step 2: download chart blob
        blob_url = f"{base_url}/v2/{project}/{chart_name}/blobs/{digest}"
        resp = requests.get(blob_url, auth=auth, verify=False, timeout=300, stream=True)
        if resp.status_code != 200:
            raise HTTPException(status_code=500, detail="Chart blob download failed")

        # Step 3: save to temp file and return
        temp_dir = tempfile.mkdtemp(prefix="chart_download_")
        chart_file = os.path.join(temp_dir, f"{chart_name}-{real_version}.tgz")
        ...
        safe_error_handler(LOG_PUSH_CHART, e, "Chart download error")
@router.get("/download_image")
async def download_image(
    image_name: str = Query(..., description="Image name"),
    image_tag: str = Query(..., description="Image tag"),
):
    """Download image from Harbor via docker pull + docker save"""
    try:
        conn = get_harbor_connection_info(require_auth=True, project_key="project")
        username = conn["username"]
        password = conn["password"]
        ...
        def _query_tags(repo):
            api = f"{agreement}{harbor_addr_local}/api/v2.0/projects/{project}/repositories/{repo}/artifacts?page_size=100"
            r = requests.get(api, auth=(username, password), verify=False, timeout=30,
                             headers={"Accept": "application/json"})
            if r.status_code != 200:
                return None
            tags = []
        ...
        # --- candidate real image names from the tar filename ---
        base = image_name
        ...
        if base:
            cands.append(base)
        b2 = base
        for arch in ("-amd64", "-arm64", "-arm", "-x86_64"):
            if b2.endswith(arch):
                b2 = b2[: -len(arch)]
                break
        if b2 and b2 not in cands:
            cands.append(b2)
        b3 = b2
        if image_tag and b3.endswith("-" + image_tag):
            b3 = b3[: -(len(image_tag) + 1)]
        if b3 and b3 not in cands:
            cands.append(b3)

        real_name = None
        real_tag = image_tag
        # --- try each candidate repo against Harbor tags ---
        for c in cands:
            try:
                tags = _query_tags(c)
            except Exception as e:
                logging.warning("[%s] tag query failed for %s: %s", LOG_PUSH, c, e)
                tags = None
            if tags:
                real_name = c
                if image_tag and image_tag in tags:
                    real_tag = image_tag
                else:
                    cand_tag = None
                    if image_tag:
                        for t in tags:
                            if image_tag in t or t in image_tag:
                                cand_tag = t
                                break
                    real_tag = cand_tag or tags[0]
                break

        # --- fallback: search all repos in project by tag ---
        if not real_name:
            try:
                rl = f"{agreement}{harbor_addr_local}/api/v2.0/projects/{project}/repositories?page_size=100"
                rr = requests.get(rl, auth=(username, password), verify=False, timeout=30,
                                  headers={"Accept": "application/json"})
                if rr.status_code == 200:
                    for repo_obj in rr.json():
                        repo = repo_obj.get("name", "")
                        if "/" in repo:
                            repo = repo.split("/", 1)[1]
                        tags = _query_tags(repo)
                        if not tags:
                            continue
                        if image_tag and image_tag in tags:
                            real_name = repo
                            ...
                            break
                        for t in tags:
                            if image_tag and (image_tag in t or t in image_tag):
                                real_name = repo
                                real_tag = t
                                break
                        if real_name:
                ...
                logging.warning("[%s] repo search failed: %s", LOG_PUSH, e)

        if not real_name:
            real_name = base or image_name

        target_image = f"{harbor_addr_local}/{project}/{real_name}:{real_tag}"

        # Step 1: docker login
        login_cmd = ["docker", "login", harbor_url, "-u", username, "--password-stdin"]
        run_command(login_cmd, timeout=30, input_str=password, log_prefix=LOG_PUSH)

        # Step 2: docker pull
        run_command(["docker", "pull", target_image], timeout=600, log_prefix=LOG_PUSH)

        # Step 3: docker save
        temp_dir = tempfile.mkdtemp(prefix="image_download_")
        tar_file = os.path.join(temp_dir, f"{real_name}-{real_tag}.tar")
        run_command(["docker", "save", "-o", tar_file, target_image], timeout=600, log_prefix=LOG_PUSH)

        # Step 4: cleanup local image
        try:
            subprocess.run(["docker", "rmi", target_image], capture_output=True, timeout=30)
        except Exception:
            pass
        ...
        safe_error_handler(LOG_PUSH, e, "Image download error")


def _harbor_delete_repo(username, password, project, repo, tag, api_base="https://127.0.0.1"):
    """Delete artifact by tag then repo if empty. Return (ok, message)."""
    import urllib.parse
    base = api_base
    headers = {"Accept": "application/json"}
    try:
        quoted = urllib.parse.quote(tag, safe="")
        art_url = f"{base}/api/v2.0/projects/{project}/repositories/{repo}/artifacts/{quoted}"
        r = requests.delete(art_url, auth=(username, password), verify=False, timeout=30)
        if r.status_code not in (200, 202, 404):
            return False, f"delete artifact failed: HTTP {r.status_code} {r.text[:200]}"
        lst = requests.get(f"{base}/api/v2.0/projects/{project}/repositories/{repo}/artifacts?page_size=100",
                           auth=(username, password), verify=False, timeout=30, headers=headers)
        if lst.status_code == 200:
            artifacts = lst.json()
            if not artifacts:
                d = requests.delete(f"{base}/api/v2.0/projects/{project}/repositories/{repo}",
                                    auth=(username, password), verify=False, timeout=30)
                if d.status_code not in (200, 202, 404):
                    return False, f"delete repo failed: HTTP {d.status_code} {d.text[:200]}"
        return True, "ok"
    except Exception as e:
        return False, str(e)
...
async def delete_image(
    ...
    """Delete image from Harbor repository (artifact + repo)."""
    try:
        conn = get_harbor_connection_info(require_auth=True, project_key="project")
        username = conn["username"]
        password = conn["password"]
        ...
        harbor_addr_local = conn["harbor_addr_local"]
        project = conn["project"]
        ok, msg = _harbor_delete_repo(username, password, project, image_name, image_tag)
        if ok:
            return {"code": 200, "message": "Image deleted", "data": {"image_name": image_name, "image_tag": image_tag}}
        return {"code": 500, "message": msg}
    except HTTPException:
        raise
    except Exception as e:
        safe_error_handler(LOG_PUSH, e, "Image delete error")
...
async def delete_chart(
    ...
    """Delete chart from Harbor repository (artifact + repo)."""
    try:
        conn = get_harbor_connection_info(require_auth=True, project_key="template")
        username = conn["username"]
        password = conn["password"]
        ...
        harbor_addr_local = conn["harbor_addr_local"]
        project = conn["project"]
        ok, msg = _harbor_delete_repo(username, password, project, chart_name, chart_version)
        if ok:
            return {"code": 200, "message": "Chart deleted", "data": {"chart_name": chart_name, "chart_version": chart_version}}
        return {"code": 500, "message": msg}
    except HTTPException:
        raise
    except Exception as e:
        safe_error_handler(LOG_PUSH_CHART, e, "Chart delete error")
        return {"code": 500, "message": str(e)}