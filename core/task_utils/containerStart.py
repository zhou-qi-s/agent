...
从 Harbor 仓库拉取 Helm Chart 模板并部署到 k8s/k3s 集群。
"""
    ...
    return {
        ...
        "error": (
            ...
            if success
                ...
                "error_type": error_type or "ContainerStartError",
...
def _get_harbor_config() -> tuple:
    """
    读取 Harbor 连接配置，返回用于 helm pull 的信息。
...
def _helm_install(
    ...
    """
    使用 helm install 部署 Chart 到 k8s/k3s 集群。

    Args:
        timeout: helm --timeout，单位为秒；为 None/0 则不传（用 helm 默认 5m）
    ...
    if timeout and int(timeout) > 0:
        install_cmd += ["--timeout", f"{int(timeout)}s"]
    if atomic:
        install_cmd.append("--atomic")

    logging.info(
        "[%s] 部署 Chart: release=%s, namespace=%s, timeout=%s, atomic=%s, kubeconfig=%s",
        _LOG, release_name, namespace, timeout, atomic, kubeconfig,
    )
    return _run_command(install_cmd, timeout=300, env=env)
...
def container_start_task(
    parameters: Dict[str, Any],
    retry: int = 0,
    timeout: int = 300,
) -> Dict[str, Any]:
    """
    从 Harbor 仓库拉取 Helm Chart 并部署到 k8s/k3s 集群。
    ...
    流程:
        ...
        3. 使用临时 kubeconfig 执行 helm install 部署到集群
    ...
    try:
        ...
        # ── Step 2: helm install 部署到集群 ──
        success, stdout, stderr = _helm_install(
        ...
        if not success:
            return _build_result(
                task_id, False, f"helm install 失败: {stderr}",
                data=base_data,
                error_type="HelmInstallError",
                error_message=stderr,
            )

        logging.info("[%s] Chart 部署成功: %s", _LOG, chart_file_name)

        return _build_result(task_id, True, f"Chart 部署成功: {chart_file_name}", data={
            **base_data,
            "status": "deployed",
            "helm_install_stdout": stdout,
        })

    except Exception as e:
        _cleanup_temp_files(kubeconfig_path, save_path)
        logging.error("[%s] 部署异常: %s", _LOG, e)
        return _build_result(
            task_id, False, f"部署异常: {e}",
            data=base_data,
...
if __name__ == "__main__":
    ...
    result = container_start_task({
        "task_id": "test-container-start-001",
