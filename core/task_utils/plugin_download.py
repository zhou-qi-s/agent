...
def plugin_download_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    # ── 5. 拼接文件名并下载到临时位置 ──
    # 与虚拟机下载一致：先下到临时目录，解压读出 app.yaml 的 name/version 后再归位。
    ...
    #       此处若按 app.yaml 改名会导致定位失败。
    if meta_name and meta_name != file_name:
        logging.warning("[插件下载] app.yaml 应用名(%s) 与平台应用名(%s) 不一致，"
                        "以平台为准归位", meta_name, file_name)
    if meta_version and meta_version != version:
        logging.warning("[插件下载] app.yaml 版本号(%s) 与平台版本号(%s) 不一致，"
    ...
    try:
        ...
        logging.error("[插件下载] 归位失败: %s", e, exc_info=True)
        cleanup_dir(staging_dir)
        return _build_download_result(task_id, False,
                                      f"整理目录结构失败: {e}",
                                      data={
