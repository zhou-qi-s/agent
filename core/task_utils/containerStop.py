...
def container_stop_task(
    ...
    try:
        ...
        if helm_timeout and int(helm_timeout) > 0:
            uninstall_cmd += ["--timeout", f"{int(helm_timeout)}s"]

        logging.info(
            "[%s] 卸载 release: %s, namespace: %s, timeout: %s, kubeconfig: %s",
            _LOG, release_name, namespace, helm_timeout, kubeconfig_path,
        )
