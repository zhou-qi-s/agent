...
与 rollback.py 结构完全对齐，唯一区别：路径多一层 plugin/（由 _SUB_DIR 承载）。
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if _is_zombie_or_dead(pid):
                logging.info(
                    "[plugin_rollback_task] PID %d 处于僵尸/死亡态，判定为已停止", pid
                )
                return False
            ...
            if expected_start_ticks is not None:
                ...
                if current_start is None:
                    logging.info(
                        "[plugin_rollback_task] PID %d 的 starttime 已不可读（进程正在退出），"
                        "判定为已停止", pid,
                    )
                    return False
                if current_start != expected_start_ticks:
                    logging.warning(
                        "[plugin_rollback_task] PID %d 启动时间已变化 (之前=%s, 现在=%s)，"
...
def plugin_rollback_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    logging.info(
        "[plugin_rollback_task] service=%s, current_version=%s, target_version=%s",
    ...
    if current_version:
        ...
        if alive_pids:
            ...
            if pid_start_ticks is not None:
                logging.info("[plugin_rollback_task] 旧进程启动时间戳: %s", pid_start_ticks)

            # 执行 stop 脚本
            logging.info("[plugin_rollback_task] 执行停止脚本: %s", stop_script_path)
            stop_exec = _execute_script(stop_script_path, old_bin_dir, min(timeout, 60))
            logging.info(
                "[plugin_rollback_task] 停止脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
            ...
            logging.info("[plugin_rollback_task] 进程 %s 已销毁", pid_before)
            was_running = True

            # 清空运行状态（pids/processes 清空，runtime 置 false）
            refresh_state_pids(service_name, [], [], _SUB_DIR)
        else:
            logging.info("[plugin_rollback_task] 当前版本未运行，跳过停止步骤")
            steps.append({
                "step": "stop_current",
                "success": True,
        ...
        logging.info("[plugin_rollback_task] 无当前版本，跳过停止步骤")
        steps.append({
            "step": "stop_current",
            "success": True,
            "message": "无当前版本，跳过停止",
    ...
    logging.info("[plugin_rollback_task] current 软链接已指向: %s", target_version)
    steps.append({
        "step": "switch_version",
        "success": True,
    ...
    # 原本未运行的服务，回滚后也保持未运行。
    if not was_running:
        logging.info("[plugin_rollback_task] 原版本未运行，跳过启动步骤")
        steps.append({
            "step": "start_target",
            "success": True,
    ...
    logging.info("[plugin_rollback_task] 使用目标版本启动脚本: %s", target_start_script)

    # 执行 start 脚本
    logging.info("[plugin_rollback_task] 执行目标版本启动脚本: %s", target_start_script)
    start_exec = _execute_script(target_start_script, target_bin_dir, min(timeout, 60))
    logging.info(
        "[plugin_rollback_task] 启动脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
        start_exec["exit_code"], start_exec.get("stdout", ""), start_exec.get("stderr", ""),
    )

    if not start_exec["success"]:
        # 启动失败 → 把 current 切回原版本（软链接回切，只需一步）
        logging.error("[plugin_rollback_task] 目标版本启动失败，尝试把 current 切回原版本 %s",
