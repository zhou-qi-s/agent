...
与 stop.py 结构完全对齐，唯一区别：路径多一层 displayConsole/（由 _SUB_DIR 承载）。
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if _is_zombie_or_dead(pid):
                logging.info(
                    "[xkt_stop_task] PID %d 处于僵尸/死亡态，判定为已停止", pid
                )
                return False
            # ── 检查2: starttime 比对 ──
            ...
            if expected_start_ticks is not None:
                ...
                if current_start is None:
                    logging.info(
                        "[xkt_stop_task] PID %d 的 starttime 已不可读（进程正在退出），"
                        "判定为已停止", pid,
                    )
                    return False
                if current_start != expected_start_ticks:
                    logging.warning(
                        "[xkt_stop_task] PID %d 启动时间已变化 (之前=%s, 现在=%s)，"
...
def xkt_stop_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    logging.info("[xkt_stop_task] 组件目录: %s, 版本: %s, bin目录: %s", component_dir, version, bin_dir)
    ...
    if pid_list:
        ...
        logging.info("[xkt_stop_task] 读取到 pids=%s（来自 state 状态文件）", pid_list)
        if pid_start_ticks is not None:
            logging.info("[xkt_stop_task] 目标进程启动时间戳: %s", pid_start_ticks)
    else:
        logging.info("[xkt_stop_task] 状态文件中无 pid，可能服务未启动")
    ...
    logging.info(
        "[xkt_stop_task] 脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
    ...
    if pid_list:
        ...
        logging.info("[xkt_stop_task] 所有进程 %s 已销毁", pid_before)
    else:
        logging.info("[xkt_stop_task] 未找到 PID 文件，可能服务未启动，跳过进程校验")
    ...
    if pid_file and os.path.isfile(pid_file):
        try:
            os.remove(pid_file)
            logging.info("[xkt_stop_task] pid 文件已删除: %s", pid_file)
        except Exception as e:
            logging.warning("[xkt_stop_task] 删除 pid 文件失败: %s", e)
