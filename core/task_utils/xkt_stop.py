...
def _build_result(
    ...
    return {
        ...
        "task_id": task_id,
        "task_type": "xkt_stop",
        # 5=已停止 / 14=停止失败（原为 6，而 6 在平台枚举里是“升级中”，
        # 会让“停止失败”在页面上显示成“升级中”）
        ...
        "error": (
            ...
            if success
                ...
                "error_type": error_type or "XktStopTaskError",
...
def _resolve_paths(service_name: str, sub_dir: str = "displayConsole") -> tuple:
    """
    根据 service_name 解析组件路径（多一层 displayConsole）。
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if _is_zombie_or_dead(pid):
                logging.info(
                    "[xkt_stop] PID %d 处于僵尸/死亡态，判定为已停止", pid
                )
                return False
            ...
            if expected_start_ticks is not None:
                ...
                if current_start is not None and current_start != expected_start_ticks:
                    logging.warning(
                        "[xkt_stop] PID %d 启动时间已变化 (之前=%s, 现在=%s)，"
...
def xkt_stop_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    """
    显控台停止服务任务（执行 bin/stop.sh 脚本）。
    ...
    logging.info("[xkt_stop] displayConsole 组件目录: %s, 版本: %s, bin目录: %s", component_dir, version, bin_dir)
    ...
    if os.path.isfile(pid_file):
        try:
            ...
            logging.info("[xkt_stop] 读取到 PID 文件内容: %s", content)
            if content:
                for line in content.splitlines():
                    ...
                    try:
                        ...
                        logging.warning("[xkt_stop] 忽略非法 PID 行: %s", line)
            if pid_list:
                pid_before = ",".join(str(p) for p in pid_list)
                pid_start_ticks = _get_process_start_ticks(pid_list[0])
                if pid_start_ticks is not None:
                    logging.info("[xkt_stop] 目标进程启动时间戳: %s", pid_start_ticks)
        except Exception as e:
            logging.warning("[xkt_stop] 读取 PID 文件失败: %s", e)
    ...
    logging.info(
        "[xkt_stop] 脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
    ...
    if pid_list:
        ...
        logging.info("[xkt_stop] 所有进程 %s 已销毁", pid_before)
    else:
        logging.info("[xkt_stop] 未找到 PID 文件，可能服务未启动，跳过进程校验")

    # ── 删除 PID 文件 ──
    if os.path.isfile(pid_file):
        try:
            os.remove(pid_file)
            logging.info("[xkt_stop] PID 文件已删除: %s", pid_file)
        except Exception as e:
            logging.warning("[xkt_stop] 删除 PID 文件失败: %s", e)
...
if __name__ == "__main__":
    ...
    if data:
        for key in ("service_name", "version", "status", "pid", "process_alive", "pid_file", "pid_file_deleted", "exit_code", "bin_dir", "script_path"):
