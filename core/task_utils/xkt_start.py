...
与 start.py 结构完全对齐，唯一区别：路径多一层 displayConsole/（由 _SUB_DIR 承载）。
...
def _read_script(bin_dir: str) -> tuple:
    ...
    if content is None:
        try:
            ...
            logging.error("[xkt_start_task] 读取启动脚本失败: %s, %s", script_path, e)
            return "", ""
...
def xkt_start_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    logging.info("[xkt_start_task] 组件目录: %s, 版本: %s, bin目录: %s", component_dir, version, bin_dir)
    ...
    logging.info("[xkt_start_task] 启动脚本已读取: %s", script_path)
    ...
    # 注意：显控台服务在运行区多一层 displayConsole/，路径必须带 _SUB_DIR，
    ...
    try:
        ...
        logging.info("[xkt_start_task] 已确保 runtime/state 目录存在: %s | %s",
    ...
    logging.info(
        "[xkt_start_task] 脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
    ...
    logging.info("[xkt_start_task] 读取到 PID 列表: %s，等待 %s 秒确认存活",
    ...
    logging.info("[xkt_start_task] PID %s 存活确认通过", pids)
    ...
    if not write_state(service_name, state, _SUB_DIR):
        logging.warning("[xkt_start_task] 写入状态文件失败，但不影响服务已启动的事实")
    logging.info("[xkt_start_task] 运行状态已写入: pids=%s processes=%s", pids, processes)

    # ── 探测监听端口（仅用于上报展示，不再写文件）──
    listen_ports = _get_listening_ports(pids[0]) if pids else []
    if listen_ports:
        logging.info("[xkt_start_task] 监听端口: %s", listen_ports)
