...
与 uninstall.py 结构完全对齐，唯一区别：路径多一层 displayConsole/（由 _SUB_DIR 承载）。
...
def xkt_uninstall_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    if is_running(service_name, _SUB_DIR) and alive_pids:
        ...
        logging.error("[xkt_uninstall_task] 服务仍在运行，拒绝卸载: pids=%s", alive_pids)
    ...
    try:
        ...
        logging.info("[xkt_uninstall_task] 运行区已清理: %s", cleared)
    except Exception as e:
        logging.warning("[xkt_uninstall_task] 清理运行区失败（不影响卸载结果）: %s", e)
