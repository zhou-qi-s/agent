...
与 start.py 结构完全对齐，唯一区别：路径多一层 plugin/（由 _SUB_DIR 承载）。
...
def _resolve_paths(service_name: str, version: str) -> tuple:
    ...
    # ── 1. 校验缓存区版本目录存在 ──
    cache_version_dir = find_cache_version_dir(service_name, version, _SUB_DIR)
    ...
    if not component_dir:
        expected_apps = os.path.join(_APPS_BASE, _SUB_DIR, service_name)
        return (
            _build_result(
...
def _read_script(bin_dir: str) -> tuple:
    ...
    if content is None:
        try:
            ...
            logging.error("[plugin_start_task] 读取启动脚本失败: %s, %s", script_path, e)
            return "", ""
...
def plugin_start_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    """
    ...
    logging.info("[plugin_start_task] 组件目录: %s, 版本: %s, bin目录: %s", component_dir, version, bin_dir)
    ...
    logging.info("[plugin_start_task] 启动脚本已读取: %s", script_path)
    ...
    # 注意：插件服务在运行区多一层 plugin/，路径必须带 _SUB_DIR，
    #       否则会在 {apps}/{service_name}/ 下建出无用的空壳目录（历史上出现过）
    runtime_dir = os.path.join(_APPS_BASE, _SUB_DIR, service_name, "runtime")
    # pid 文件放在 state/ 下，与状态文件统一管理：{apps}/{_SUB_DIR}/{service_name}/state/pid
    # 由包内 start.sh 负责写入（echo $! > .../state/pid）
    pid_file = get_pid_file(service_name, _SUB_DIR) or os.path.join(
        _APPS_BASE, _SUB_DIR, service_name, "state", "pid")

    # 预先创建 runtime 与 state 目录（脚本可能只写文件不建目录）
    try:
        os.makedirs(runtime_dir, exist_ok=True)
        os.makedirs(os.path.dirname(pid_file), exist_ok=True)
        logging.info("[plugin_start_task] 已确保 runtime/state 目录存在: %s | %s",
    ...
    logging.info(
        "[plugin_start_task] 脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
    ...
    logging.info("[plugin_start_task] 读取到 PID 列表: %s，等待 %s 秒确认存活",
    ...
    logging.info("[plugin_start_task] PID %s 存活确认通过", pids)
    ...
    if not write_state(service_name, state, _SUB_DIR):
        logging.warning("[plugin_start_task] 写入状态文件失败，但不影响服务已启动的事实")
    logging.info("[plugin_start_task] 运行状态已写入: pids=%s processes=%s", pids, processes)

    # ── 探测监听端口（仅用于上报展示，不再写文件）──
    listen_ports = _get_listening_ports(pids[0]) if pids else []
    if listen_ports:
        logging.info("[plugin_start_task] 监听端口: %s", listen_ports)
