"""
core/task_utils/plugin_start.py - 插件启动模块

通过 bin/ 目录下的启动脚本启动显控服务。
路径: download/plugin/{service_name}/{version}/bin/start.sh
"""
...
def _resolve_paths(service_name: str, sub_dir: str = "plugin") -> Tuple[Optional[Dict[str, Any]], str, str, str]:
    """
    解析 plugin 组件路径。
...
def _read_script(bin_dir: str) -> Tuple[str, str]:
    ...
    if content is None:
        try:
            ...
            logging.error("[plugin_start] 读取启动脚本失败: %s, %s", script_path, e)
            return "", ""
...
#
# 说明：原先还额外写一份 download/plugin_pid/{service_name}，但全仓库无任何
# 读取方（孤儿文件），已废弃 —— 与显控台保持一致，统一写 runtime/pid。
# =============================================================================
...
def _write_runtime_pid(pid_file: str, pid_list: List[int]) -> bool:
    ...
    try:
        ...
        logging.info("[plugin_start] 已回写 PID 文件: %s, PIDs=%s", pid_file, pid_list)
        return True
    except Exception as e:
        logging.warning("[plugin_start] 回写 PID 文件失败: %s -> %s", pid_file, e)
        return False
...
def plugin_start_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    """
    插件启动任务 — 从 bin/ 目录读取 start.sh(start.bat) 并执行。
    ...
    流程:
        1. 解析 plugin/{service_name}/ 组件目录
        2. 读取 version 文件获取版本号
        3. 从 {version}/bin/ 读取启动脚本
        4. 执行启动脚本
        5. 按进程名扫描真实在跑进程（优先），回写 {version}/runtime/pid
    ...
    logging.info("[plugin_start] 组件目录: %s, 版本: %s, bin目录: %s", component_dir, version, bin_dir)
    ...
    logging.info("[plugin_start] 启动脚本已读取: %s", script_path)
    ...
    logging.info(
        "[plugin_start] 脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
    ...
    if os.path.isfile(pid_file):
        try:
            ...
            logging.info("[plugin_start] 主进程 PID: %s", pids)
        except Exception as e:
            logging.warning("[plugin_start] 读取 PID 文件失败: %s", e)

    # ── 以"实际在跑的进程"为准，回写 {版本}/runtime/pid（唯一落点）──
    # 服务自带脚本写的 runtime/pid 可能未刷新或是旧值，故优先用按进程名扫到的结果；
    # 扫不到则沿用文件内容，交由 plugin 巡检线程后续修正
    scanned_pids = collect_service_pids(component_dir, version_dir)
    if scanned_pids:
        pids = scanned_pids
        _write_runtime_pid(pid_file, pids)

    return _build_result(task_id, True, "插件启动成功", data={
        **base_data,
        "status": "started",
        "pids": pids,
...
if __name__ == "__main__":
    ...
    test_params = {
        "task_id": "test-plugin-start-001",
        "service_name": "test-service",
    }
    result = plugin_start_task(test_params)
    import json
