...
与 upgrade.py 结构完全对齐，唯一区别：路径多一层 plugin/（由 _SUB_DIR 承载）。
...
缓存区/运行区分离后：
    ...
    · 升级动作   → **不再下载**：升级入口（平台侧）只允许选缓存区里已下载的版本，
                   这里校验 {download}/plugin/{service_name}/{版本}/ 存在后，
...
# 注意：插件升级必须使用「插件版」的下载/安装/启动任务。
# 虚拟机版（download/install/start）的路径不含 plugin/ 子目录层
# （{download}/{name}/{version}），会把新版下到 plugin/ 之外，
# 导致后续安装定位不到版本目录。插件版路径为
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if _is_zombie_or_dead(pid):
                logging.info("[plugin_upgrade_task] PID %d 处于僵尸/死亡态，判定为已停止", pid)
                return False
            # ── 检查2: starttime 比对 ──
            ...
            if expected_start_ticks is not None:
                ...
                if current_start is None:
                    logging.info("[plugin_upgrade_task] PID %d 的 starttime 已不可读（进程正在退出），判定为已停止", pid)
                    return False
                if current_start != expected_start_ticks:
                    logging.warning(
                        "[plugin_upgrade_task] PID %d 启动时间已变化，该 PID 已被复用，原进程已停止",
                        pid,
                    )
                    return False
            return True
...
def _rollback(
    ...
    **不再删除新版本目录** —— 新版本内容与旧版本平级共存在缓存区，
    ...
    if relink.get("ok"):
        ...
        logging.info("[plugin_upgrade_task][rollback] %s", msg)
    else:
        msg = f"回滚软链接失败: {relink.get('error')}"
        logging.error("[plugin_upgrade_task][rollback] %s", msg)
    ...
    if state:
        ...
        if write_state(service_name, state, _SUB_DIR):
            ...
            msg = "运行状态写入失败"
            logging.warning("[plugin_upgrade_task][rollback] %s", msg)
        rollback_steps.append(msg)
    else:
...
def plugin_upgrade_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    ...
    logging.info("[plugin_upgrade_task] 当前版本: %s, 新版本: %s", current_version or "(无)", new_version)
    ...
    logging.info("[plugin_upgrade_task] 跳过下载，直接安装缓存区版本: %s", cache_version_dir)
    ...
    if current_version:
        ...
        if alive_pids:
            ...
            logging.info("[plugin_upgrade_task] 执行停止脚本: %s", stop_script)
            stop_exec = _execute_script(stop_script, bin_dir, min(timeout, 60))
            logging.info(
                "[plugin_upgrade_task] 停止脚本执行完成, exit_code=%s, stdout=%s, stderr=%s",
        ...
        logging.info("[plugin_upgrade_task] 无当前版本，首次安装，跳过停止步骤")
        steps.append({
            "step": "stop_old",
            "success": True,
            "message": "无旧版本，跳过停止",
