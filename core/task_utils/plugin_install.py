...
与 install.py 结构完全对齐，唯一区别：路径多一层 plugin/
"""
...
def plugin_install_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    """
    插件安装任务 — 定位已下载的显控组件并执行 bin/install.sh 安装脚本。
    ...
    流程:
        ...
        2. 进入 {base_path}/plugin/{file_name}/ 读取 version 文件
        3. 定位 {base_path}/plugin/{file_name}/{version}/bin/install.sh
        4. 执行安装脚本
    """
    ...
    logging.info(f"[插件安装] 基础路径: {base_path}, 组件目录: {component_dir}")
    ...
    logging.info(f"[插件安装] 组件版本: {version}")
