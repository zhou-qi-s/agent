...
与 install.py 结构完全对齐，唯一区别：路径多一层 plugin/（由 sub_dir 参数承载）。

职责：把「缓存区」中已下载的指定版本部署为「运行区」结构，再执行包内 install.sh。

缓存区（server.download）：
    {download}/plugin/{app_name}/{version}/{app,bin,config}

运行区（server.apps）：
    {apps}/plugin/{app_name}/
        ├── state/config.yaml      运行状态（pids / processes / name / runtime / version）
        ├── current -> {download}/plugin/{app_name}/{version}
        ├── app     -> current/app
        ├── bin     -> current/bin
...
处理流程：
    1. 校验参数（version 必传）
    2. 在缓存区查找 {download}/plugin/{file_name}/{version}/ 版本目录
    ...
    return {
        ...
        "error": (
            ...
            if success
                ...
                "error_type": error_type or "PluginInstallTaskError",
...
def plugin_install_task(parameters: Dict[str, Any], retry: int = 0, timeout: int = 300) -> Dict[str, Any]:
    """
    插件安装任务 — 把缓存区的指定版本部署到运行区，并执行 bin/install.sh。
    ...
    sub_dir = str(parameters.get("sub_dir", "") or SUB_DIR_PLUGIN).strip()
    ...
    if not version_dir:
        ...
        logging.error(f"[插件安装] 版本目录不存在: {expected}")
        if not os.path.isdir(component_dir):
            return _build_result(
                ...
                error_message=f"下载目录下未找到插件 {file_name}，请先执行下载任务",
            )
    ...
    logging.info(f"[插件安装] 组件目录: {component_dir}, 版本目录: {version_dir}")
    ...
    if apps_component_dir:
        ...
        logging.info(f"[插件安装] 检测到运行区已存在: {apps_component_dir}, 状态={state}")

        if is_running(file_name, sub_dir):
            running_ver = state.get("version", "")
            running_pids = state.get("pids", [])
            logging.error(
                f"[插件安装] 应用正在运行，拒绝安装: version={running_ver} pids={running_pids}"
            )
            return _build_result(
                task_id, False,
        ...
        if state.get("pids") or state.get("pid"):
            logging.info(f"[插件安装] 应用未运行，清空残留进程记录: {state.get('pids')}")
        state["pids"] = []
        state["processes"] = []
        state.pop("pid", None)
    else:
        logging.info(f"[插件安装] 运行区尚无该应用，将新建: {file_name}")
    ...
    logging.info(f"[插件安装] 软链接建立完成: {app_dir}")
    ...
    logging.info(f"[插件安装] 运行状态已更新: {state}")
    ...
    try:
        ...
        if result.success:
            logging.info(f"[插件安装] 安装脚本执行成功: {script_path}")
            return _build_result(
                task_id, True, "安装成功",
                data={
                    "status": "installed",
            ...
            logging.error(f"[插件安装] 安装脚本执行失败: exit_code={result.code}, stderr={result.stderr}")
            return _build_result(
        ...
        logging.error(f"[插件安装] 安装脚本执行异常: {e}")
        return _build_result(
...
if __name__ == "__main__":
    ...
    test_file_name = "demo-plugin"
    test_version = "1.0.0"

    result = plugin_install_task(
        {
            "task_id": "test-plugin-install-001",
